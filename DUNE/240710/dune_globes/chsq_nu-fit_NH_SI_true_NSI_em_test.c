...
{
  char MYFILE[] = "chsq_nu-fit_NH_SI_true_em_test.dat";
  ...
  /* NSI */
  double true_eps_e_m = 0; // SI for true
  double true_phi_e_m = 0;
  ...
  for (double test_eps_e_m = 0; test_eps_e_m <= 1.0; test_eps_e_m += 0.025)
  {
        ...
        for (double marg_theta12 = 0.275; marg_theta12 <= 0.344; marg_theta12 += (0.344 - 0.275) / 3)
        {
          for (double marg_deltacp = 139; marg_deltacp <= 350; marg_deltacp += 20)
          {
            for (double marg_sdm = 6.81e-5; marg_sdm <= 8.03e-5; marg_sdm += (8.03e-5 - 6.81e-5) / 3)
            {
              ...
              {
                for (double marg_phi_e_m = 0; marg_phi_e_m <= 360; marg_phi_e_m += 45)
                {
                  ...
                  double the_phi_e_m = marg_phi_e_m * my_M_PI / 180;

                  glbDefineParams(test_values, thetheta12, thetheta13, thetheta23, thedeltacp, marg_sdm, marg_ldm);
                  glbSetDensityParams(test_values, 1.0, GLB_ALL);
                  glbSetOscParams(test_values, test_eps_e_m, GLB_EPS_E_M);
                  glbSetOscParams(test_values, the_phi_e_m, GLB_PHI_E_M);
