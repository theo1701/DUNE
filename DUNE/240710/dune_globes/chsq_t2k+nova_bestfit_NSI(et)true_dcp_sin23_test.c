...
{
  char MYFILE[] = "chsq_t2k+nova_bestfit_NSI(et=0.15,NH)true_dcp_sin23_test.dat";
  ...
  double true_theta23 = asin(sqrt(0.55));
  double true_sdm = 7.34e-5;
  double true_ldm = 2.485e-3;

  double true_deltacp = 1.46 * my_M_PI / 180;
  ...
  for (double test_deltacp = -180; test_deltacp <= 180; test_deltacp += 10) {
    for (double test_sin23 = 0.41; test_sin23 <= 0.61; test_sin23 += 0.1) {
      m = 1e10;
      for (double marg_eps_e_t = 0; marg_eps_e_t <= 1; marg_eps_e_t += 0.1) {
        for (double marg_phi_e_t = -180; marg_phi_e_t <= 180; marg_phi_e_t += 10) {
          ...
          glbDefineParams(test_values, true_theta12, true_theta13, thetheta23, thedeltacp, true_sdm, true_ldm);
